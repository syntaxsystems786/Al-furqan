...
export default function AdminProducts() {
  ...
  const openEdit = (product: Product) => {
    ...
    setImagePreviews([
      ...
      product.images?.[1] ? product.images[1].url : null
    ]);
    setShowModal(true);
  };
                  ...
                  <tr key={product.id} className="hover:bg-[#0B0B0B] transition-colors group">
                    <td className="p-5">
                      <div className="flex items-center space-x-4">
                        {product.images?.[0] ? (
                          <img src={product.images[0].url} alt="" width={48} height={60} className="object-cover bg-[#0B0B0B] border border-[#D4AF37]/20 aspect-[4/5]" style={{width:48,height:60}} />
                        ) : (
